...
import {
	fetchAllProducts,
	refreshProducts as refreshGoogleSheets,
...
const fallbackPhoneData: Phone[] = [...simuData, ...viooData].filter(
	(item): item is Phone => item.price !== null && item.price !== undefined
);

export function AppProvider({ children }: { children: ReactNode }) {
	const [allPhoneData, setAllPhoneData] =
		useState<Phone[]>(fallbackPhoneData);
	const [isLoadingProducts, setIsLoadingProducts] = useState(false);

	// Load products from Google Sheets on mount
	useEffect(() => {
	...
	const refreshProducts = async () => {
		setIsLoadingProducts(true);
		try {
			const products = await refreshGoogleSheets();
			if (products && products.length > 0) {
				setAllPhoneData(products);
			}
		} catch (error) {
	...
	const addToCart = (phone: Phone) => {
		setCart((prevCart) => {
			const existingItem = prevCart.find(
				(item) =>
					item.phone.brand === phone.brand &&
					item.phone.model === phone.model
			);
			if (existingItem) {
